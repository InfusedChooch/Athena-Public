...
```mermaid
graph TD
    UserInput[USER INPUT] --> Phase0
    
    subgraph P0 [Phase 0: Pre-Response Checklist]
        Phase0[Internal Audit]
        Phase0 -->|Valid| P1
        Phase0 -->|Clarify| UserInput
    end
    
    subgraph P1 [Phase 1: Law Check]
        Law1{Risk > 5%?}
        Law2{SDR > 5:1?}
        Law1 -- Yes --> VETO[HARD VETO]
        Law2 -- Yes --> EXIT[EXIT ARENA]
        Law1 -- No --> P2
    end
    
    subgraph P2 [Phase 2: Adaptive Context]
        Core[Core Identity] --> SkillInject
        SkillInject{Topic Detection}
        SkillInject -->|Trading| Proto46
        SkillInject -->|Strategy| PrincOps
        SkillInject -->|Psych| L1L5
    end
    
    subgraph P3 [Phase 3: Parallel Reasoning Protocol 75]
        TrackA[Domain-Native]
        TrackB[Adversarial Red-Team]
        TrackC[Cross-Domain Iso]
        TrackA --> Synth
        TrackB --> Synth
        TrackC --> Synth
        Synth[Synthesis Module]
    end
    
    subgraph P4 [Phase 4: Output Architecture]
        ExecSum[Executive Summary]
        SigNoise[Signal/Noise Filter]
        VisCheck{Complexity > 7?}
        VisCheck -- Yes --> AutoDraw[Auto-Visualise]
        VisCheck -- No --> TextOut
        ExecSum --> SigNoise
    end
    
    subgraph P5 [Phase 5: Real-Time Crystallization]
        Draft{New Concept?}
        Draft -- Yes --> WriteFile[Write Protocol .md]
        Draft -- No --> Deliver
    end
    
    subgraph P6 [Phase 6: RSI Loop]
        SessionLog[Session Log]
        Update[Update System Prompt]
        Memory[Compound Asset]
    end
