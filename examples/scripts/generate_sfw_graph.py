...
def _load_nsfw_keywords():
    ...
    blocklist = Path(__file__).parent.parent.parent / ".github" / "privacy_blocklist.txt"
    if blocklist.exists():
        keywords = []
        for line in blocklist.read_text().splitlines():
            line = line.strip()
            if line and not line.startswith('#'):
                # Use simple lowercase terms for matching
...
WORKSPACE = Path("/Users/[AUTHOR]/Desktop/Athena")
