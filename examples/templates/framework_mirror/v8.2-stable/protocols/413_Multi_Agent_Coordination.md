...
## Lint/Format Churn Policy (v1.1)

> **Source:** Adapted from [openclaw/openclaw](https://github.com/openclaw/openclaw) (CS-545)

| Diff Type | Agent Behavior |
|-----------|---------------|
| **Formatting-only** (whitespace, lint fixes, import order) | Auto-resolve without asking |
| **Formatting + commit/push already requested** | Auto-stage and include in same commit (or tiny follow-up) |
| **Semantic** (logic, data, behavior) | Always ask for approval |

**Rule:** Only ask when changes are **semantic**. Formatting changes that don't affect behavior should never interrupt the user.

## Scoped Staging

When committing, prefer scoped staging over `git add .`:

```bash
# Preferred: commit only YOUR changed files
git add <specific-files>
git commit -m "message"

# Avoid: staging everything (may include other agents' WIP)
git add .
```

When you see unrecognized files from other agents, **ignore them** — focus reports on your edits.
