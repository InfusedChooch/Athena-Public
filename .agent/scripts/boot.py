...
def main():
    ...
    """
    # Deterministic /start telemetry (TD-057): the prompt hook only fires on
    # slash-prefixed prompts in Claude Code, so boot.py is the reliable producer.
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from log_invocation import log_invocation

        log_invocation("workflow", "start", trigger="auto: boot.py")
    except Exception:
        pass
